...
void sum_of_subsets(int s, int k, int rem)
{
	...
	{
//if subset found
		printf("subset = %d\n", ++count); for(int i=0 ; i <= k ; i++)
		if ( x[i] == 1) printf("%d ",w[i]);
		printf("\n");
	}
	else if ( s + w[k] + w[k+1] <= d )
	//left tree evaluation 
...
int main()
{
	...
	scanf("%d",&n);
	printf("enter the elements in increasing order:"); 		for( int i = 0; i < n ; i++)
	{
		scanf("%d",&w[i]);
		sum=sum+w[i];
	}
	...
	scanf("%d",&d);
	if ( ( sum < d ) || ( w[0] > d ) ) printf("No subset possible\n");
	else sum_of_subsets(0,0,sum);
}
